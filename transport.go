...
func (t *Transport) Clone() *Transport {
	...
	if t.TLSExtensions != nil {
		tlsExt, _ := t.TLSExtensions.Clone()
		t2.TLSExtensions = tlsExt
	}
	t2.JA3 = t.JA3
...
func (t *Transport) roundTrip(req *Request) (_ *Response, err error) {
	...
	if isHTTP {
		...
		if err := validateHeaders(req.Header); err != "" {
			req.closeBody()
			return nil, fmt.Errorf("github.com/wangluozhe/chttp: invalid header %s", err)
		}

		// Validate the outgoing trailers too.
		if err := validateHeaders(req.Trailer); err != "" {
			req.closeBody()
			return nil, fmt.Errorf("github.com/wangluozhe/chttp: invalid trailer %s", err)
		}
	}
	...
	if req.Method != "" && !validMethod(req.Method) {
		req.closeBody()
		return nil, fmt.Errorf("github.com/wangluozhe/chttp: invalid method %q", req.Method)
	}
	if req.URL.Host == "" {
		req.closeBody()
...
var errCannotRewind = errors.New("github.com/wangluozhe/chttp: cannot rewind body after connection loss")
...
func (t *Transport) dial(ctx context.Context, network, addr string) (net.Conn, error) {
	if t.DialContext != nil {
		...
		if c == nil && err == nil {
			err = errors.New("github.com/wangluozhe/chttp: Transport.DialContext hook returned (nil, nil)")
		}
		return c, err
	}
	if t.Dial != nil {
		...
		if c == nil && err == nil {
			err = errors.New("github.com/wangluozhe/chttp: Transport.Dial hook returned (nil, nil)")
		}
		return c, err
	}
...
func (w *wantConn) tryDeliver(pc *persistConn, err error, idleAt time.Time) bool {
	...
	if (pc == nil) == (err == nil) {
		panic("github.com/wangluozhe/chttp: internal error: misuse of tryDeliver")
	}
	w.ctx = nil
	w.done = true
...
func (t *Transport) decConnsPerHost(key connectMethodKey) {
	...
	if n == 0 {
		...
		panic("github.com/wangluozhe/chttp: internal error: connCount underflow")
	}
...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod, isClientConn bool, internalStateHook func()) (pconn *persistConn, err error) {
	...
	if unencryptedHTTP2 {
		...
		alt := next(cm.targetAddr, unencryptedTLSUConn(pconn.conn))
...
func (pc *persistConn) mapRoundTripError(req *transportRequest, startBytesWritten int64, err error) error {
	...
	if pc.isBroken() {
		...
		return fmt.Errorf("github.com/wangluozhe/chttp: HTTP/1.x transport connection broken: %w", err)
	}
	return err
}
...
func (pc *persistConn) readLoop() {
	...
	for alive {
		...
		if err != nil {
			if pc.readLimit <= 0 {
				err = fmt.Errorf("github.com/wangluozhe/chttp: server response headers exceeded %d bytes; aborted", pc.maxHeaderResponseSize())
			}
...
var errTimeout error = &timeoutError{"github.com/wangluozhe/chttp: timeout awaiting response headers"}

// errRequestCanceled is set to be identical to the one from h2 to facilitate
// testing.
var errRequestCanceled = internal.ErrRequestCanceled
var errRequestCanceledConn = errors.New("github.com/wangluozhe/chttp: request canceled while waiting for connection") // TODO: unify?
...
var errRequestDone = errors.New("github.com/wangluozhe/chttp: request completed")
...
func (pc *persistConn) roundTrip(req *transportRequest) (resp *Response, err error) {
	...
	if !pc.t.DisableCompression &&
		!headerKeyExists(req.Header, "Accept-Encoding") &&
		!headerKeyExists(req.Header, "Range") &&
		req.Method != "HEAD" {
...
// headerKeyExists checks whether any key in the header matches the given key
// case-insensitively. This handles the case where users set headers with
// non-canonical keys (e.g., "accept-encoding" instead of "Accept-Encoding").
func headerKeyExists(h Header, key string) bool {
	// Fast path: check canonical key directly
	if _, ok := h[key]; ok {
		return true
	}
	// Slow path: iterate and compare case-insensitively
	for k := range h {
