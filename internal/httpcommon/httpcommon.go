...
func EncodeHeaders(ctx context.Context, param EncodeHeadersParam, headerf func(name, value string)) (res EncodeHeadersResult, _ error) {
	...
	enumerateHeaders := func(f func(name, value string)) {
		...
		// [RFC3986]).
		pHeaderOrder, ok := req.Header["PHeader-Order:"]
		m := req.Method
		if m == "" {
			m = "GET"
		}
		...
		for k, vv := range req.Header {
			if asciiEqualFold(k, "host") || asciiEqualFold(k, "content-length") {
				...
				// Per 8.1.2.2 Connection-Specific Header Fields
				continue
				...
				localHeaders[k] = vv
			} else if k == ":protocol" || k == "PHeader-Order:" || k == "Header-Order:" || k == "UnChanged-HeaderKey:" {
				continue
			} else {
				localHeaders[k] = vv
			}
		...
		if shouldSendReqContentLength(req.Method, req.ActualContentLength) {
			localHeaders["content-length"] = []string{strconv.FormatInt(req.ActualContentLength, 10)}
		}
		if param.AddGzipHeader {
			localHeaders["accept-encoding"] = []string{"gzip"}
		}
		...
		if !hasUA && param.DefaultUserAgent != "" {
			localHeaders["user-agent"] = []string{param.DefaultUserAgent}
		}
		...
		if headerOrder, ok := req.Header["Header-Order:"]; ok {
			orderMap := make(map[string]int)
			for i, header := range headerOrder {
				orderMap[strings.ToLower(header)] = i
			}
			sort.Slice(keys, func(i, j int) bool {
				idxi, iok := orderMap[strings.ToLower(keys[i])]
				idxj, jok := orderMap[strings.ToLower(keys[j])]
				if !iok && !jok {
...
func validateHeaders(hdrs map[string][]string) string {
	for k, vv := range hdrs {
		if k == "PHeader-Order:" || k == "Header-Order:" || k == "UnChanged-HeaderKey:" {
			continue
		}
