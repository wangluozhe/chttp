...
import (
	...
	"github.com/wangluozhe/chttp/internal/http2"
...
func (t *Transport) configureHTTP2(protocols Protocols) {
	...
	if t.TLSNextProto == nil {
		t.TLSNextProto = make(map[string]func(authority string, c *tls.UConn) RoundTripper)
	}
	t.TLSNextProto["h2"] = func(authority string, c *tls.UConn) RoundTripper {
	...
	t.TLSNextProto[nextProtoUnencryptedHTTP2] = func(authority string, c *tls.UConn) RoundTripper {
...
// Aliased HTTP/2 fingerprint types for external consumption.
type HTTP2Setting = http2.Setting
type HTTP2SettingID = http2.SettingID
type HTTP2PriorityFrame = http2.PriorityFrame
type HTTP2PriorityParam = http2.PriorityParam
type HTTP2FrameHeader = http2.FrameHeader
type HTTP2Settings = http2.HTTP2Settings
type HTTP2Transport = http2.Transport

// Aliased HTTP/2 SettingID constants for external consumption.
const (
	HTTP2SettingHeaderTableSize      HTTP2SettingID = http2.SettingHeaderTableSize
	HTTP2SettingEnablePush           HTTP2SettingID = http2.SettingEnablePush
	HTTP2SettingMaxConcurrentStreams HTTP2SettingID = http2.SettingMaxConcurrentStreams
	HTTP2SettingInitialWindowSize    HTTP2SettingID = http2.SettingInitialWindowSize
	HTTP2SettingMaxFrameSize         HTTP2SettingID = http2.SettingMaxFrameSize
	HTTP2SettingMaxHeaderListSize    HTTP2SettingID = http2.SettingMaxHeaderListSize
)

// HTTP2ConfigureTransports creates an HTTP/2 Transport for the given HTTP/1 Transport
// and wires them together. It returns the HTTP/2 Transport for further configuration.
func HTTP2ConfigureTransports(t1 *Transport) (*HTTP2Transport, error) {
	if t1.TLSClientConfig == nil {
		t1.TLSClientConfig = &tls.Config{}
	}
	if t1.HTTP2 == nil {
		t1.HTTP2 = &HTTP2Config{}
	}
	t2 := http2.NewTransport(transportConfig{t1})
	t2.AllowHTTP = true
	t1.H2Transport = t2

	t1.registerProtocol("https", http2RoundTripper{t2, true})
	if t1.TLSNextProto == nil {
		t1.TLSNextProto = make(map[string]func(authority string, c *tls.UConn) RoundTripper)
	}
	t1.TLSNextProto["h2"] = func(authority string, c *tls.UConn) RoundTripper {
		err := t2.AddConn("https", authority, c)
		if err != nil {
			return http2ErringRoundTripper{err}
		}
	...
	t1.TLSNextProto[nextProtoUnencryptedHTTP2] = func(authority string, c *tls.UConn) RoundTripper {
		unencrypted, ok := c.NetConn().(unencryptedNetConnInTLSConn)
		if !ok {
			return http2ErringRoundTripper{errors.New("http: *tls.Conn expected to wrap an unencrypted conn, but does not (BUG)")}
		}
	...
	hasH2 := false
	for _, p := range t1.TLSClientConfig.NextProtos {
		if p == "h2" {
			hasH2 = true
			break
		}
